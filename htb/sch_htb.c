...
static char *traced_iface="eth0";
module_param    (traced_iface, charp, 0);
MODULE_PARM_DESC(traced_iface, "Interface to trace TCP packet enqueues/dequeues");
...
static int htb_enqueue(struct sk_buff *skb, struct Qdisc *sch)
{
	...
	if (cl == HTB_DIRECT) {
        ...
        q->ofbuf_queued++;
        if(skb->dev != NULL)
            trace_mn_htb("ofbuf", skb->dev->name, q->ofbuf_queued);
    ...
    if(skb->dev != NULL)
        trace_mn_htb("enqueue", skb->dev->name, 1+sch->q.qlen);
...
ok:
        ...
        if(skb->dev != NULL)
            trace_mn_htb("dequeue", skb->dev->name, sch->q.qlen);
#if OFBUF
        if(q->ofbuf_queued > 0) {
            i = 0;
